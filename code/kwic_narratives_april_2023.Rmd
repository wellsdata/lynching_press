...
### Import Data
```{r}
#load extracted text: 298564 rows
lynch <- read_csv("https://osf.io/download/gw5dk/?view_only=6c106acd6cb54f6f849e8c6f9098809f")

# lynch <- read.csv("~/Code/hcij_lynching_phase_two/narratives/data/articles_oct_19.csv")

#index of extracted articles (formely jackindex): /hcij_lynching_phase_two/narratives/data/master_article_index_10.19.csv jackindex <- read.csv("~/Code/hcij_lynching_phase_two/narratives/data/master_article_index_10.19.csv") %>% 
#   mutate(doc_id = paste0(file_id,"_",article_id,".txt"))

index_extracted <- read.csv("https://osf.io/download/uxw3a/?view_only=6c106acd6cb54f6f849e8c6f9098809f") |> 
  mutate(doc_id = paste0(file_id,".txt"))

#index of all 60,042 articles captured
...
lynch1 <- readtext("~/Code/lynching_press/data/articles_june_22_2024")
...
lynch1 <- lynch1 %>% 
   inner_join(index_extracted, by=c("doc_id"))
...
### Ida Wells
```{r}
ida_b <- kwic(lynch_tokens, phrase(c("ida b wells", "ida b. wells", "ida wells", "miss wells", "wells barnett", "barnett")), window = 50, valuetype = "regex") %>% as.data.frame() 

ida_b <- ida_b %>% 
  inner_join(index_extracted, by=c("docname"="doc_id"))
```
...
mob_analysis <- mob_analysis %>% 
  inner_join(index_extracted, by=c("docname"="filename"))
...
row_values <- 1:nrow(index_extracted)
