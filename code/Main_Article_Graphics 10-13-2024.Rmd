...
#Figure 6: Detail_ida_graph_10_13_2024
#Fixed 10-13-2024
```{r}
...
#subset the 1633 Black press news articles
...
```{r}
#uses the main index of 11,223  extracted articles
...
#REDO with extracted articles
```{r}
#Using the 11,223  extracted articles, we determine the number of page 1 stories per decade. 

extracted_articles_index_june_22_2024$decade <- paste0(substr(extracted_articles_index_june_22_2024$year, 0, 3), "0")

x_extracted_articles_index_june_22_2024 <- extracted_articles_index_june_22_2024 %>% 
  select(newspaper_name, newspaper_state, sn, year, month, day, decade, edition,page, filepath)

index_pages <- x_extracted_articles_index_june_22_2024 %>% 
  mutate(page_one = ifelse(page > 1, FALSE, TRUE))
...
count_decade <- extracted_articles_index_june_22_2024 %>% 
  group_by(decade) %>%
  count() %>% 
    ungroup() %>% 
  mutate(pct_total = n/(sum(n))) %>% 
  ...
  add_header_above(c("Lynching Articles by Decade, n=11,223 " = 3)) %>% 
...
pageplacement <- extracted_articles_index_june_22_2024 %>% 
  # mutate(page = str_replace(page, "seq-", "")) %>% 
  group_by(page) %>% 
  count(page) %>% 
  ungroup()
...
pageplacement %>% 
  ...
  labs(title = "Page Placement, Lynching Coverage, 1789-1963", 
       ...
       caption = "Page 1 stories were 34% of 11,223  articles. Graphic by (redcated) , 6/26/2024",
       y="Pct of Pages",
       x="Page Number")
...
#Downloads sample of 3,225 articles manually coded by a team of undergraduate researchers, Fall 2023.
