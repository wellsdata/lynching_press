...
#index of 11,396 extracted articles, a 19% sample which has been checked by a coder and represents all valid entries
...
library(kableExtra)
articles_decades %>%
  kbl(caption = "LOC Lynching Articles by Decade (n=11,396, 6/21/2024)", font_size = 30) %>%
...
vizDataFrame <- vizDataFrame %>% 
  mutate(category = case_when(
    str_detect(variable, "lynch mob negro murder crime law men hang attempt made") ~ "lynch_law",
    str_detect(variable, "peopl south great race countri american good state public excit") ~ "unclear_topic",
    str_detect(variable, "bodi men jail mob fire hang crowd shot tree hous") ~ "lynch_mob",
    str_detect(variable, "negro white man kill murder year assault mrs woman charg") ~ "lynching_violence",
        #check this
    str_detect(variable, "state court law governor juri order case unit judg offic") ~ "legal_proceedings",
      #check this
    str_detect(variable,  "counti night jail citi morn day sheriff mile town clock") ~ "unknown_mob_gathering?",
  ))
...
Here are the topics:
1: lynch law: lynch mob negro murder crime law men hang attempt made	
2: unclear_topic: peopl south great race countri american good state public excit
3: lynch_mob: bodi men jail mob fire hang crowd shot tree hous
4: lynching_violence: negro white man kill murder year assault mrs woman charg
#check this
5: legal proceedings: state court law governor juri order case unit judg offic
6: lynchings: counti night jail citi morn day sheriff mile town clock



Cut after fc - previous topics
...
ggplot(vizDataFrame, aes(x=decade, y=value, fill=category)) + 
  ...
  labs(title = "Common Narratives in Lynching News Coverage",
       ...
       caption = "Aggregate mean topic proportions per decade. Graphic by (redated - peer review) & (redated - peer review), 6-27-2023")
