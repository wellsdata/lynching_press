...
```
How many Black papers in the 59,9967 articles
Index creation of black press from LOC index
### Create flag for Black papers in index
```{r}
#index of 331 (formerly 76) black articles from LOC
blackLOC<- jackindex %>%
 ...
 blackLOC <- blackLOC %>% 
   select(file_id2, newspaper_name, date, year, month, day, page, edition, mod_id, url, newspaper_state, filename, filepath, file_id, article_id, sn, Collection) 
...
blackindex_master <- blackindex %>% 
  full_join(blackLOC, by=c("file_name"="filename", "collection"="Collection", "url", "date", "newspaper_state", "year", "filepath", "newspaper_name"))
