...
### Updated Beck-Tolnay Victim Data - Compare Trends with News Coverage

The University of Illinois holds the Updated Beck-Tolnay Victim_Data of all known lynching cases from 1882-1929. 5,872 cases, 1865-2020, probable and confirmed lynchings.
...
tolnay_counts %>%
 ...
 #kbl(caption = "Lynching Totals", font_size = 30) %>%
  kbl(caption = "Lynching by type, Updated Beck-Tolnay list of 5,872 cases, 1865-2020", font_size = 24) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>% 
  column_spec(1, bold = T, border_right = T) %>%
  column_spec(2, width = "5em", background = "yellow") 

#Updated Beck Tolnay Totals 11_17_2024.png
...
tolnay_graphic <- tolnay_counts %>%
  arrange(desc(Number)) %>% 
  kbl(caption = "Lynching Totals, Updated Beck-Tolnay, 2022", font_size = 30) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>% 
  column_spec(1, bold = T, border_right = T) %>%
  column_spec(2, width = "5em", background = "yellow") 
...
webshot(temp_html, file = img_file, zoom = 15) # Adjust zoom to control resolution 
...
Year Totals - beck_tolnay
```{r}
tolnay_beck_year <- tolnay_beck %>% 
 filter(status_clean =="lynching") |> # added nov 17
  group_by(year) %>% 
  count(year) %>% 
  rename(total = n) %>% 
  mutate(pct_total = formattable::percent(total/5039, 3)) # added nov 17
 #mutate(pct_total = formattable::percent(total/5871, 3))
tolnay_beck_year
```

Focus on 1868 lynching
```{r}
tolnay_beck |> 
  filter(year=="1868") |> 
  count(lynch_state) |> 
...
pl <- pl + labs(title = "Newspaper Coverage vs Lynchings, 1865-1963", 
       ...
       caption = "Tolnay_Beck Victims n = 5039. Media n = 11,194  articles. Graphic by (redacted - peer review), 11/17/2024")

ggsave("../output_images_tables/Article_Images/Figure_5_ida_graph_11_17_2024.png", device = "png",width=9,height=6, dpi=800)

pl
```

#Figure 6: Detail ida graph 11 17 2024
...
ggsave("../output_images_tables/Article_Images/Figure_6_detail_ida_graph_11_17_2024.png", device = "png",width=9,height=6, dpi=800)
...
write.csv(mob_summary, "../Code/lynching_press/output_images_tables/mob_summary_10_20_2024.csv")
